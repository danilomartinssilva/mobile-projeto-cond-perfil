...
export function* update({sugestion}) {
  try {
    const response = yield call(
      api.put,
      'sugestions/' + sugestion.id,
      sugestion,
    );

    yield put({
      type: '@sugestions/UPDATE_SUCESS',
      sugestion: response.data,
    });

    goBack();
  } catch (failed) {
    yield put({type: '@sugestions/UPDATE_FAILURE', failed});
    const message = responder.failed(failed);
    toast(message);
  }
}
...
export default all([
  ...
  takeLatest('@sugestions/UPDATE_REQUEST', update),
]);
