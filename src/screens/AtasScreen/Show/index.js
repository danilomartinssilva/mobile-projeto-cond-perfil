...
export default function AtasShowScreen({navigation, route}) {
  const minutes = useSelector((state) => state.minutes);

  const dispatch = useDispatch();
  const {id} = route.params;
  useEffect(() => {
    dispatch(Minutes.showMinuteRequest(id));
  }, []);
  useLayoutEffect(() => {
  ...
  return (
    <Container>
      {!!minutes.detail && !!minutes.detail.file && (
        <PDFView
          fadeInDuration={250.0}
          style={{flex: 1}}
          resource={minutes.detail.file.url}
          resourceType={'url'}
